...
mGBA emulator
  └─ Lua script (codex_mailbox_bridge.lua)
       ├─ polls  gPokeliveCodexMailbox (EWRAM 0x0203F48C) every frame
...
|---|---|---|
| `gPokeliveCodexMailbox` | `0x0203F48C` | Set in `codex_mailbox_bridge.lua` line 8 |
| `gPokelivePartyData` | `0x0203F5BC` | Set in `codex_mailbox_bridge.lua` line 9 |
...
## Mailbox Struct Layout (offset from 0x0203F48C)
...
|---|---|---|
| 0 | Compile patched ROM, get mailbox address | ✅ Done — ROM built, address `0x0203F48C` |
...
| 2b | **Verify EWRAM addresses** — re-grep `.map`, update Lua lines 8–9 if shifted | ✅ Done — addresses unchanged (`0x0203F48C`, `0x0203F5BC`) |
| 2c | **E2E test** — ADVICE with real party, ASK with typed input | 🔲 Pending |
| 3 | **AI Rival** — pre-battle AI planning, counter pick, move scoring, battle history | 🔲 Planned (see PLAN.md) |
| 4 | Custom Pokémon generator (stretch goal) | 🔲 Pending |
