...
u8 BattleAI_ChooseMoveOrAction(void)
{
    ...
    // Pokelive: Smart Gary move-score boost from AI bridge plan.
    {
        if (gRivalAIBuffer.active && gRivalAIBuffer.magic == POKELIVE_RIVAL_AI_BUFFER_MAGIC)
        {
            s32 j;
            for (j = 0; j < MAX_MON_MOVES; j++)
            {
                s8 boost = gRivalAIBuffer.moveScore[j];
